...
} from "../ai/inkeep-qa-tools-schema";
import { z } from "zod";
...
export default function IntelligentForm() {
	...
	const [escalationFormCaption, setEscalationFormCaption] =
		...
		setEscalationFormCaption(
			caption || (
				<p className="text-gray-600 text-sm">
					To finish submitting a support ticket, confirm the fields below and
					click Submit.
				</p>
			),
		);
	...
	const handleAIResponses = ({
		...
		answerConfidence: z.infer<
			typeof ProvideAIAnnotationsToolSchema
		...
		if (
			(answerConfidence === "very_confident" ||
				answerConfidence === "somewhat_confident") &&
			answer &&
			links &&
			links.length > 0
		) {
	...
	const onClickNext = async () => {
		...
		try {
			const { qaModelResponse, contextModelResponse } = await invokeInkeepAI(
				formData.message,
			);
										...
										<div className="space-y-8">
											...
											<div className="space-y-1">
												...
												<Select
													...
													onValueChange={handleSelectChange("ticketType")}
												>
													<SelectTrigger>
														<SelectValue placeholder="Select ticket type" />
													</SelectTrigger>
													<SelectContent>
														<SelectItem value="talk_to_sales">
															Talk to sales
														</SelectItem>
														<SelectItem value="issue_in_production">
															Issue in production
														</SelectItem>
														<SelectItem value="issue_in_development">
															Issue in development
														</SelectItem>
														<SelectItem value="report_bug">
															Report bug
														</SelectItem>
														<SelectItem value="onboarding_help">
															Onboarding help
														</SelectItem>
